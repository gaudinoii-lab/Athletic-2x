  ...
  function exerciseHtml(ex, ei) {
    ...
    return `<article class="exercise-card ${doneCount === ex.sets ? 'completed' : ''}">
      <div class="exercise-head">
        <div class="exercise-title-wrap"><span class="item-title"><h3>${escapeHtml(ex.name)}</h3>${guideLinkHtml(ex.guide, ex.name)}</span><p>${ex.sets} × ${escapeHtml(ex.reps)} · recupero ${formatRest(ex.rest)}${ex.note ? ` · ${escapeHtml(ex.note)}` : ''}</p></div>
